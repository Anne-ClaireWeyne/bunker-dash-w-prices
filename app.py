...
def dict_filter(dictionary,portCode,grade):
    return dictionary["portCode"]==portCode and dictionary["grade"]==grade

##API reader
def json_reader():
    response=requests.get("https://app.bunker-ex.com/api/prices/v1/getAllLivePrices/{}".format(BE_KEY))
    response_json=response.json()
    return response_json

response_json=json_reader()


def fuel_price(fr="01-01-2014",to="01-06-2019",port=["full"],type_vessel=["full"],
               size=["full"],*args):
    '''Bunker Ex API machinery'''
  
    reading=response_json["now"]
    reading=datetime.datetime.strptime(reading.split(".",1)[0],"%Y-%m-%dT%H:%M:%S").strftime("%Y-%m-%d %H:%M:%S")+"UTC"
    ...
    portCode=df_in.code.iloc[0]
    
    filtered_ifo=[d for d in response_json["prices"] if dict_filter(d,portCode,"IFO380")]
    
    if len(filtered_ifo)>0:
        ifo_price="$"+" "+str(round(filtered_ifo[0]["currentPrice"],2))
    else:
        ifo_price="Not Available"
    
    filtered_vlsfo=[d for d in response_json["prices"] if dict_filter(d,portCode,"VLSFO")]

    if len(filtered_vlsfo)>0:
        vlsfo_price="$"+" "+str(round(filtered_vlsfo[0]["currentPrice"],2))
    else:
        vlsfo_price="Not Available"
    
    return html.Div([html.Div(html.H5("{}".format(port_name))),
         html.Div([html.Div([html.H3("IFO 380"),html.H4(ifo_price)]),
         html.Div([html.H3("VLSFO"),html.H4(vlsfo_price)]),
         html.Div([html.H3("Time"),html.H4(children=reading[0],style={"font-size":"14px"}),html.H4(children=reading[1],style={"font-size":"18px"})]),
         html.Div([html.H4("Prices by:"),
                    html.A(
                    className="div-logo",
                    children=html.Img(
                        className="logo",src=app.get_asset_url("bunker_ex.png")),
                    href="https://www.bunker-ex.com")]
                  )],className="box-summary2")]
                    ,className="box-summary",id="fuel_price")
...
app.layout = html.Div(children=[dcc.ConfirmDialog(id='date-error',message='Wrong date range'),
                      ...
                      html.Div(id="main-stats-price",className="container four columns",
                                children=[
                                   ...
                                   dcc.Interval("interval-component",interval=43200*1000,n_intervals= 0)],className="main-box")##12 hrs interval
...
##Prices update
@app.callback(Output("fuel_price","children"),
              [Input('update-button',"n_clicks"),
                Input('ports-dropdown', 'value'),
                      Input("types-dropdown", "value"),
                      Input('date-picker-start', 'date'),
...
def fuel_price_update(click,ports_val,types_val,date_s,date_e,size,time):
    if not ports_val:
        ports_val=["full"]
    if not types_val:
        types_val=["full"]
    ...
    if click is not None:
        ...
        return fuel_price(fr=date_s,to=date_e,port=ports_val,
                        type_vessel=types_val,size=size)     
